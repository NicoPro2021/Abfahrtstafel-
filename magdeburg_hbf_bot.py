...
def hole_daten():
    ...
    try:
        # ID für Magdeburg Hbf suchen
        suche_url = "https://v6.db.transport.rest/locations?query=Magdeburg+Hbf&results=1"
        ...
        # Abfahrten laden (Magdeburg Hbf ist ein großer Knoten, daher results=30)
        ...
        for d in departures:
            try:
                line = d.get('line', {})
                # Filter: Nur Züge/S-Bahnen, keine Busse/Straßenbahnen
                if line.get('product') in ['bus', 'tram', 'ferry']: continue
...
if __name__ == "__main__":
    daten = hole_daten()
    with open('magdeburg_hbf.json', 'w', encoding='utf-8') as f:
