...
def hole_daten():
    ...
    try:
        # ID für Roßlau(Elbe) suchen
        suche_url = "https://v6.db.transport.rest/locations?query=Rosslau(Elbe)&results=1"
        ...
        # Abfahrten laden (Roßlau ist ein Knotenpunkt, daher results=20)
        ...
        for d in departures:
            try:
                ...
                for rm in remarks:
                    if rm.get('type') == 'hint':
                        t = rm.get('text', '').strip()
                        # Unwichtige Standardinfos ausblenden
        ...
        return res[:12] # Zeige bis zu 12 Züge an
    except:
        return []

if __name__ == "__main__":
    daten = hole_daten()
    with open('rosslau.json', 'w', encoding='utf-8') as f:
