...
# STARK ERWEITERTE DB-CODES FÜR MEHR ZUG- UND BAHNHOFS-INFOS
DB_CODES = {
    ...
    "11": "Verspätung eines vorausfahrenden Zuges", "15": "Bauarbeiten", 
    "16": "Bauarbeiten", "18": "Defekt am Zug", "21": "Türstörung",
    "38": "Defekt an der Klimaanlage", "43": "Kurzfristiger Personalausfall",
    "46": "Verspätung eines vorausfahrenden Zuges", "70": "Kein WLAN",
    "73": "Mehrzweckabteil fehlt", "76": "Fahrradmitnahme nicht möglich",
    "77": "Ohne 1. Klasse", "80": "Andere Wagenreihung", "82": "Mehrere Wagen fehlen",
    "83": "Störung an der fahrzeuggebundenen Rampe", "84": "Zug verkehrt richtig gereiht",
    "85": "Ein Wagen fehlt", "86": "Kein Reservierungsdisplay",
    "87": "Einzelne Wagen ohne Klimaanlage", "88": "Keine Mehrzweck-Toiletten",
    "89": "Fahrradmitnahme nur eingeschränkt möglich", "90": "Kein Halt an diesem Bahnhof",
    "91": "Fahrradmitnahme nicht möglich", "92": "Technische Störung am Zug",
    "93": "Gestörte behindertengerechte Einrichtung", "94": "Ersatzbewirtschaftung",
    "95": "Ohne behindertengerechtes WC", "96": "Zug ist stark überbesetzt",
    "97": "Zug ist überbesetzt", "98": "Sonstige Störungen"
}
    ...
    try:
        ...
        for s in root.findall('s'):
            ...
            if dp is not None and tl is not None:
                ...
                # --- NEU: Geplante Informationen aus dem Basis-Fahrplan holen ---
                # Sucht nach <m> Tags im gesamten Zug-Eintrag
                plan_msgs = [DB_CODES.get(m.get('c'), f"Info {m.get('c')}") for m in s.findall('.//m') if m.get('c')]
                ...
                # --- NEU: Alle Meldungen (Geplant + Echtzeit) zusammenführen ---
                echtzeit_msgs = chg.get('msgs', [])
                alle_meldungen = plan_msgs + echtzeit_msgs
                
                # Doppelte Einträge entfernen (behält die Reihenfolge bei)
                alle_meldungen = list(dict.fromkeys(alle_meldungen))
                begruendung = " | ".join(alle_meldungen)
                
                # Schöner formatierte Linie (z.B. "ICE 123" statt "ICE123")
                linien_name = dp.get('l') or f"{tl.get('c', '')} {tl.get('n', '')}".strip()
    ...
    try:
        ...
        if c_res.status_code == 200:
            for s in ET.fromstring(c_res.content).findall('s'):
                ...
                # --- NEU: Sucht ALLE <m> Tags rekursiv im gesamten <s> Block (.//m) ---
                msgs = [DB_CODES.get(m.get('c'), f"Info {m.get('c')}") for m in s.findall('.//m') if m.get('c')]
                
                changes[s.get('id')] = {
                    "ct": dp.get('ct') if dp is not None else None,
                    "cp": dp.get('cp') if dp is not None else None,
                    "cs": dp.get('cs') if dp is not None else None,
                    "msgs": msgs # Speichern als Liste, um sie später besser zusammenzuführen
                }
    except: pass
...
def hole_routing_verbindungen(start_id, ziel_id, dateiname):
    ...
    try:
        ...
        print(f"Timeout-Fehler beim Routing für {dateiname}: VBB Server braucht zu lange.")
...
if __name__ == "__main__":
    ...
    # Paralleler Abruf der Bahnhöfe mit max. Geschwindigkeit (5 Threads)
